...
ngOnInit(): void {
  ...
  updateProfile() {
    ...
    let ccExpiry = this.customer?.creditCardExpiry;
    if (ccExpiry) {
      ccExpiry =
        ccExpiry.substring(0, 1) + values.expiryDate + ccExpiry.substring(5);
    } else {
      ccExpiry = `1${values.expiryDate}01`;
    }
  ...
  getDate() {
    let date ="09/11/2024 08:06:03";
     date = this.profileForm.controls.dateOfBirth.value==null?
     "09/11/2024 08:06:03":
